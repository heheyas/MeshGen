...
class RenderConfig:
    ...
    # fov_para: float = (
    #     np.pi / 3 * 0.9
    # )  # 0.61 or 0.8 for Orthographic ; np.pi / 3 for Pinhole
    fov_para: float = np.deg2rad(36)
    ...
    render_angle_thres: float = 70
    # Suzanne
    views_init: List[float] = field(default_factory=[0, 23].copy)
    views_inpaint: List[Tuple[float, float]] = field(
        default_factory=[(5, 6), (24, 25)].copy
    )
...
class Renderer:
    def __init__(self, mesh_face_num, device, **kwargs):
        render_cfg = RenderConfig(**kwargs)
        self.device = device
        ...
        assert self.interpolation_mode in [
            "nearest",
            "bilinear",
            "bicubic",
        ], f"no interpolation mode: {self.interpolation_mode}"
        assert self.calcu_uncolored_mode in [
            "WarpGrid",
            "FACE_ID",
            "DIFF",
        ], f"no uncolored mask caculation mode: {self.calcu_uncolored_mode}"
        assert project_mode in [
            "Pinhole",
            "Orthographic",
        ], f"no projecttion mode: {project_mode}"

        if project_mode == "Pinhole":
            self.intrinsics = kal.render.camera.PinholeIntrinsics.from_fov(
                width=render_cfg.grid_size,
                height=render_cfg.grid_size,
                fov=fov_para,
                device=device,
            )
        elif project_mode == "Orthographic":
            self.intrinsics = kal.render.camera.OrthographicIntrinsics.from_frustum(
                width=render_cfg.grid_size,
                height=render_cfg.grid_size,
                near=-800,
                far=800,
                fov_distance=fov_para,
                device=device,
            )
    ...
    def get_camera_from_view(self, theta, phi, radius):
        ...
        camera_transform = kal.render.camera.generate_transformation_matrix(
            pos, look_at, direction
        )
        return camera_transform

    def normalize_depth(self, depth_map, min_val=0.5):
        assert (
            depth_map.max() <= 0.0
        ), "depth map should be negative"  # 相机视野中不存在物体, 就会报错
        ...
        if not isinstance(min_val, float):
            if min_val == "raw":
                ...
                disp = depth_map[object_mask]
                disp = -1 / (disp + 1e-20)
                disp = (disp - disp.min()) / (disp.max() - disp.min() + 1e-20)
                depth_map[object_mask] = disp
                return depth_map
        elif min_val < 0.0:
            object_mask = depth_map != 0
            min_val = -min_val
    ...
    def forward_texturing_render(
        ...
        normal_map, _ = kal.render.mesh.rasterize(
            texture_dims[0],
            texture_dims[1],
            face_vertices_z,
            uv_face_attr * 2 - 1,
        ...
        uv_features_inv, face_idx_inv = kal.render.mesh.rasterize(
            texture_h,
            texture_w,
            uv3d,
            uv_face_attr * 2 - 1,
        ...
        ## test
        # _, _, H, W = view_target.shape
        # uv_features_inv, face_idx_inv = kal.render.mesh.rasterize(
        #     H,
        #     W,
        #     face_vertices_camera[:, :, :, -1],
        #     face_vertices_image,
        #     face_features=uv_face_attr * 2 - 1,
        #     valid_faces=seen_cons_from_view.to(torch.bool),
        # )

        # proj_mask = uncolored_mask.bool().squeeze(1)
        # view_target = rearrange(view_target, "b c h w -> b h w c")
        # view_target[~proj_mask] = 1.0
        # texture_mask = repeat(valid_face_mask[0], "h w 1 -> h w c", c=3)
        # uv_features_inv = uv_features_inv[proj_mask]
        # view_target = view_target[proj_mask]

        # uv_features_inv[..., 1] *= -1
        # cur_texture_map = mipmap_linear_grid_put_2d(
        #     texture_h,
        #     texture_w,
        #     uv_features_inv.view(-1, 2)[..., [1, 0]],
        #     view_target.view(-1, 3),
        #     min_resolution=32,
        # ).reshape(1, texture_h, texture_w, 3)
        # write_image("trash/cur_texture_map.png", cur_texture_map[0])
        ## test
        ...
        cur_texture_map = kal.render.mesh.texture_mapping(
            uv_features_inv, view_target, mode=interpolation_mode
        )

        cur_metalic_map = None
        if metalic_target is not None:
            cur_metalic_map = kal.render.mesh.texture_mapping(
                uv_features_inv, metalic_target, mode=interpolation_mode
            ).permute(0, 3, 1, 2)

        cur_roughness_map = None
        if roughness_target is not None:
            cur_roughness_map = kal.render.mesh.texture_mapping(
                uv_features_inv, roughness_target, mode=interpolation_mode
            ).permute(0, 3, 1, 2)

        cur_texture_map_original = cur_texture_map.clone()
        cur_texture_map = cur_texture_map * valid_face_mask
        cur_texture_update_area = kal.render.mesh.texture_mapping(
            uv_features_inv,
            uncolored_mask.repeat(1, 3, 1, 1),
        ...
        return (
            ...
            cur_metalic_map,
            cur_roughness_map,
            cur_texture_update_area.permute(
                0,
                3,
                1,
                2,
            ...
            cur_texture_map_original.permute(0, 3, 1, 2),
        )
    ...
    def render_single_view_texture(
        ...
        render_cache=None,
        dims=None,
        texture_default_color=[0.8, 0.1, 0.8],
        depth_min_val=0.5,
        ...
        if render_cache is None:
            ...
            face_features_rasterized, face_idx = kal.render.mesh.rasterize(
                dims[1],
                dims[0],
                face_vertices_camera[:, :, :, -1],
                face_vertices_image,
            ...
            depth_map = self.normalize_depth(
                face_features_rasterized[0], min_val=depth_min_val
            )
        ...
        if render_cache is None and not skip_uncolored_mask:
            if self.calcu_uncolored_mode == "WarpGrid":
                texture_map_diff = (
                    (
                        texture_map.detach()
                        - torch.tensor(texture_default_color)
                        .view(1, 3, 1, 1)
                        .to(self.device)
                    )
                    .abs()
                ...
                uncolored_mask = kal.render.mesh.texture_mapping(
                    uv_features, uncolored_texture_map, mode=self.interpolation_mode
                )
            elif self.calcu_uncolored_mode == "FACE_ID":
                check = (face_idx > -1) & (face_idx < self.mesh_face_num - 1)
                cur_seen_faces = torch.zeros(
                    1, self.mesh_face_num, 1, device=self.device
                )
                ...
                diff = (
                    (
                        image_features.permute(0, 3, 1, 2).clamp(0, 1).detach()
                        - torch.tensor(texture_default_color)
                        .view(1, 3, 1, 1)
                        .to(self.device)
                    )
            ...
            uncolored_mask = (uncolored_mask * mask + 0 * (1 - mask)).permute(
                0, 3, 1, 2
            )

        if skip_uncolored_mask:
            uncolored_texture_map = torch.ones_like(texture_map)
            uncolored_mask = kal.render.mesh.texture_mapping(
                uv_features, uncolored_texture_map, mode=self.interpolation_mode
            )
        ...
        normals_image = face_features_rasterized[2].detach()
    ...
    def render_single_view(
        ...
        #     face_normals = -face_normals @ camera_transform[0, :3, :3].T
        # else:
        #     face_normals = -face_normals
        vertex_normals = trimesh.geometry.mean_vertex_normals(
            vertex_count=verts.size(0),
        ...
        face_features_rasterized, face_idx = kal.render.mesh.rasterize(
            ...
            face_vertices_camera[:, :, :, -1],
            face_vertices_image,
            [face_vertices_camera[:, :, :, -1:], face_vertices_normals],
        )
        depth_map = self.normalize_depth(
            face_features_rasterized[0], min_val=depth_min_val
        )
        normal_map = face_features_rasterized[1].detach()

        mask = (face_idx > -1).float()[..., None]
        normal_map = normal_map * mask + 1 * (1 - mask)
